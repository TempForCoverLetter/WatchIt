agent_list = '''Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/37.0.2062.94 Chrome/37.0.2062.94 Safari/537.36
...
Mozilla/5.0 (iPhone; CPU iPhone OS 8_4 like Mac OS X) AppleWebKit/600.1.4 (KHTML, like Gecko) GSA/7.0.55539 Mobile/12H143 Safari/600.1.4'''.splitlines(
)
...
request_scarp = cloudscraper.create_scraper()
regex_magnet = r"(magnet:[^\"\']*)"
no_special_char = re.compile('[^\w ]+')
replace_space = re.compile('%20')

base_url_1337x = "https://1337x.to/sort-search/"
end_url_1337x = "/seeders/desc/1/"
'''def search_all_torrents_1337x(q):
    r = requests.get(base_url_1337x + requests.utils.quote(q) + end_url_1337x,
                     headers={'user-agent': random.choice(agent_list)})
    df = pd.read_html(r.text, flavor="lxml")
    return (df[0])'''


def search_torrent_magnet_generic_2steps(url,
                                         q,
                                         regex_page=r"(/torrent/[^\"\']*)",
                                         is_not_plus=True,
                                         is_full_adress=False,
                                         request_type=request_scarp):
    r = request_type.get(url.replace(
        "{@q@}",
        requests.utils.quote(
            no_special_char.sub('', replace_space.sub(' ', q.lower())))
        if is_not_plus else quote_plus(
            no_special_char.sub('', replace_space.sub(' ', q.lower())))),
                         headers={'user-agent': random.choice(agent_list)})
    if r.status_code == 200:
        match = re.search(regex_page, r.text)
        if (match):
            r = request_type.get(
                ("" if is_full_adress else
                 ("https://" + (urlsplit(r.url).netloc))) + (match.group()),
                headers={'user-agent': random.choice(agent_list)})
            if r.status_code == 200:
                match = re.search(regex_magnet, r.text)
                if (match):
                    return (match.group())
...
def search_torrent_file_generic_2steps(
        url,
        q,
        regex_page=r"(/torrent/[^\"\']*)",
        regex_file=r"(/itor[^\"\']*)",
    ...
    if r.status_code == 200:
        ...
        if (match):
            r = requests.get(
                ...
                headers=headers)
            if r.status_code == 200:
                match = re.search(regex_file, r.text)
                if (match):
                    return ("https:/" + add_before + match.group() + add_after)

    return ""


def search_torrent_magnet_generic(url,
                                  q,
                                  is_not_plus=True,
                                  request_type=request_scarp):
    r = request_type.get(url.replace(
...
def search_torrent_file_generic(url,
                                q,
                                regex_file=r"(/it[^\"\']*)",
                                is_not_plus=True,
                                add_before="",
...
def search_torrent_magnet_galaxy(q):
    return search_torrent_magnet_generic(
        "https://torrentgalaxy.to/torrents.php?search={@q@}&lang=1&sort=seeders&order=desc",
        q,
        request_type=requests)


def search_torrent_magnet_1337x(q):
    return search_torrent_magnet_generic_2steps(
        "https://1337x.to/sort-search/{@q@}/seeders/desc/1/",
        q,
        is_not_plus=True)


def search_torrent_magnet_torrentdownload_dot_info(q):
    return search_torrent_magnet_generic_2steps(
        "https://www.torrentdownload.info/feed?q={@q@}",
        q,
        regex_page=r"((?<=guid>)h[^<]*)",
        is_not_plus=False,
        is_full_adress=True)
...
def search_torrent_file_galaxy(q):
    return search_torrent_file_generic(
        "https://torrentgalaxy.to/torrents.php?search={@q@}&lang=1&sort=seeders&order=desc",
        q,
        regex_file=r"(/watercache[^\\'\"]*)",
        request_type=requests)


def search_torrent_file_1337x(q):
    return search_torrent_file_generic_2steps(
        "https://1337x.to/sort-search/{@q@}/seeders/desc/1/",
        q,
        is_not_plus=True)


def search_torrent_file_limetorrents(q):
    return search_torrent_file_generic(
        "https://www.limetorrents.lol/searchrss/{@q@}",
        q,
        regex_file=r"(/itor[^\"\']*)",
        is_not_plus=True)


def search_torrent_file_torrentdownload_dot_info(q):
    return search_torrent_file_generic(
        "https://www.torrentdownload.info/feed?q={@q@}",
        q,
        regex_file=r"(/[^a-z.-/]*)(?=</guid>)",
        is_not_plus=False,
        add_before="/itorrents.org/torrent",
        add_after=".torrent",
    )